<x-app-layout>
    <x-slot name="header">
        ...
        <a href="{{ route('provinces.create') }}" class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-base px-4 py-2 text-center inline-flex items-center mr-2">
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                                ...
                                @foreach ($provinces as $province)
                                    <tr class="bg-white @if (!$loop->last) border-b @endif">
                                        <td scope="row" class="px-6 py-4 font-medium text-blue-600 whitespace-nowrap ">
                                            <a href="{{ route('provinces.show', $province) }}">{{ $province->name }}</a>
                                        </td>
                                        <td class="px-6 py-4">
                                            {{ $province->slug }}
                                        </td>
                ...
                @else
                    <div class="p-4 sm:p-8 bg-white shadow overflow-hidden sm:rounded-lg">
                        <p>{{ __('page.province.index.message.no_results') }}</p>
                    </div>
                @endif
            </div>
        </div>
