<x-app-layout>
    <x-slot name="header">
        <h1 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('navigation.municipality_show') }}
        </h1>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow overflow-hidden sm:rounded-lg">
                <h2 class="mb-6 text-xl font-semibold leading-none text-gray-900 md:text-2xl">{{ $municipality->title }}</h2>

                <div class="grid grid-cols-2 grid-rows-2 gap-5 mb-5" style="flex: 0 0 auto;">
                    <dl class="block w-full">
                        <dt class="mb-2 font-semibold leading-none text-gray-900">{{ __('form.label.slug') }}</dt>
                        <dd class="font-light leading-none text-gray-500">{{ $municipality->slug }}</dd>
                    </dl>
                    ...
                    <dl class="block w-full">
                        ...
                        <dd class="font-light leading-none text-gray-500">{{ $municipality->code }}</dd>
                    </dl>

                    <dl class="block w-full">
                        <dt class="mb-2 font-semibold leading-none text-gray-900">{{ __('form.label.code_municipality_center') }}
                        </dt>
                        <dd class="font-light leading-none text-gray-500">{{ $municipality->ekatte }}</dd>
                    </dl>
                </div>

                <dl>
                    <dt class="mb-2 font-semibold leading-none text-gray-900">{{ __('form.label.description') }}</dt>
                    <dd class="mb-4 font-light leading-tight text-gray-500 sm:mb-5">{{ $municipality->description }}</dd>
                </dl>
                ...
                <dl class="flex items-center space-x-6">
                    <div>
                        ...
                        <dd class="mb-4 font-light leading-none text-gray-500 sm:mb-5">
                            {{ $municipality->created_at->format('d.m.Y, H:i:s') }}</dd>
                    </div>

                    <div>
                        <dt class="mb-2 font-semibold leading-none text-gray-900">{{ __('form.label.updated_at') }}</dt>
                        <dd class="mb-4 font-light leading-none text-gray-500 sm:mb-5">
                            {{ $municipality->updated_at->format('d.m.Y, H:i:s') }}</dd>
                    </div>
                </dl>

                <div class="flex items-center space-x-4">
                    <a href="{{ route('municipalities.edit', $municipality) }}"
                    ...
                    {{ html()->form('DELETE', route('municipalities.destroy', $municipality))->open() }}
                    <button type="submit"
                    ...
                    <a href="https://www.nsi.bg/nrnm/ekatte/municipalities" target="_blank" rel="noopener"
                        ...
                        <span>{{ __('button.nsi_municipalities') }}</span>
                    </a>
                </div>
            </div>
        </div>
