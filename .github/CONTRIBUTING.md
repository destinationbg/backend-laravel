...
За да успеете да подкарате приложението, то изисква да разполагате и със статичното съдържание за всяка забележителност. То е налично тук чрез `submodule` функционалността на Git и е позиционирано в директорията `/resources/static-contents`. Промените във файловете обаче се извършват от [static-contents](https://github.com/placesbg/static-contents) хранилището и впоследствие се синхронизират тук, така че не правете промени директно от това приложение.

Ако за първи път ще работите върху това приложение, първо трябва да [копирате нашето хранилище във вашия GitHub профил](https://github.com/placesbg/backend-laravel/fork) чрез Fork функцията. След това трябва да клонирате вашето собствено хранилище в локалната ви среда чрез `git clone` командата и да въведете следните команди в последователен ред, както е показано:

```bash
# създава вашия локален конфигурационен файл за подмодулите, за да може да следи за промени по тях впоследствие
# настройва автоматично конфигурацията, така че да можете да използвате командата git submodule update за клониране и актуализиране на подмодулите
git submodule init

# добавяне правило в submodule конфигурацията, което игнорира промените, които правим в него от това хранилище
# никога не променяйте файлове в submodule директорията от това хранилище
git config submodule.resources/static-contents.ignore all
...
# извлича и попълва в базата данни статичното съдържание за забележителностите
...
Вече може да отворите приложението от адрес `https://localhost:8000/`.
...
```bash
# извлича новостите от main branch-a на static-contents хранилището
git submodule foreach git pull origin main

# стартира локално Docker контейнер и пуска софтуера, който е нужен за работата на Laravel приложението
sail up -d

# актуализира базата данни с последните промени, които взехме от static-contents хранилището
sail artisan landmarks:update
